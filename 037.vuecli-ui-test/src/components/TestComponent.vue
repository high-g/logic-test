<template>
    ...
    <div class="area">
      ...
      <p :class="{'non-active': isHidden}">このエリアにnon-activeクラスが適応されると非表示になります。</p>
    ...
    <div class="area">
      ...
      <ul>
        <li v-for="(item, key) in list" :key="key">{{ item.user }}：{{ item.message }}</li>
      </ul>
    </div>
    <div class="area">
      <h2>リストレンダリング - 配列</h2>
      <span class="p-l10" v-for="(item, key) in arr" :key="key">{{ item }}</span>
    </div>
	</div>
</template>
